...
$action = New-ScheduledTaskAction -Execute "$(scoop prefix git)\git-bash.exe" -Argument '-l -c "source /etc/restic/default.env.sh && /bin/restic_backup.sh"'
...
$action = New-ScheduledTaskAction -Execute "$(scoop prefix git)\git-bash.exe" -Argument '-l -c "source /etc/restic/default.env.sh && /bin/restic_check.sh"'
